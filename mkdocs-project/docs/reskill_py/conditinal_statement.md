...
Pythonのプログラムは、通常は上から順番に1行ずつ実行されます。しかし、「制御構造」を使えば、特定の条件に応じて処理の流れを変えることができます。これにより、より柔軟で賢いプログラムが書けるようになります。

### 制御構造の3つの基本形

Pythonのプログラムは「文（**statement**）」の集まりです。その実行順を変える代表的な制御構造には次の3つがあります：

#### 1. 順次:
: 最も基本的な実行の流れで、上から順に処理されます。
...
!!! note "構造化定理"
    ...
    プログラムは「順次」「分岐」「繰り返し」の3つの制御構造を組み合わせれば、どんなに複雑な動作でも表現できるとされています。
